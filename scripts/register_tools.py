...
def kb_search_corporate(
    ...
    """
    Поиск в корпоративной базе знаний Респект.Чата (ai.respect_kb, 5453+ карточек).
    Использует hybrid retrieval: FTS + Vector (e5-large) + RRF + Cohere reranker.

    Используй когда вопрос пользователя касается:
    - Корпоративных регламентов, политик, должностных инструкций
    - Бухгалтерии, налогов, кадров, командировок, отпусков
    - Шпаргалок, типовых ситуаций, обзоров для бухгалтера/кадровика
    - Любых внутренних документов компании Респект.Чата

    Args:
        query: Поисковый запрос на естественном языке. Будь конкретным.
               Например: "процедура оформления командировки", "налог на прибыль 2026",
               "увольнение сотрудника", "ШПАРГАЛКА командировочные расходы".
        matrix_user_id: MXID пользователя (обязательный, для ACL-проверки).
                        Берётся из [from=...] в контексте разговора.
        matrix_room_id: ID Matrix-комнаты (опционально). Не используется новым endpoint,
                        но оставлен для совместимости.
        limit: Сколько результатов вернуть (1-20). По умолчанию 5.

    Returns:
        Форматированный текст с найденными карточками: title, snippet, score, источник
        (vector/fts/cohere), приложения. Если ничего не найдено — соответствующее сообщение.

    Когда использовать:
        - Вопросы про корпоративные правила, регламенты, процедуры
        - "У нас есть документ про X?", "Как у нас оформляется Y?"
        - Любые рабочие/профессиональные вопросы пользователя
    ...
    if not matrix_user_id:
        return "Не могу искать в корпоративной KB — нужен MXID пользователя."
    ...
    try:
        ...
        return f"Ошибка соединения с корпоративной KB: {e}"
    ...
    data = r.json()
    formatted = data.get("formatted_for_llm") or data.get("formatted")
    if formatted:
        return formatted
    return "По запросу ничего не найдено в корпоративной KB."
