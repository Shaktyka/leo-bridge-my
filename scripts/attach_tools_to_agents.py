...
def main() -> None:
    ...
    r.raise_for_status()
    cal_tools = [t for t in r.json() if t["name"].startswith("calendar_") or t["name"] in ("internet_search",) or t["name"].startswith("kb_") or t["name"].startswith("respect_") or t["name"].startswith("leo_")]
    if not cal_tools:
